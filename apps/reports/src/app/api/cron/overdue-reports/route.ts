...
export async function GET(request: Request) {
  ...
  const overdue: { specialist: string; client: string; stage: string }[] = [];
  for (const v of vprs || []) {
  ...
  const deduped = Array.from(new Map(overdue.map((m) => [`${m.specialist}|${m.client}`, m])).values());

  if (deduped.length === 0) {
    return NextResponse.json({ message: 'No overdue reports' });
  }

  const emailBody =
    'The following GVRA Monthly Reports are OVERDUE (deadline was 10th at 5:00pm):\n\n' +
