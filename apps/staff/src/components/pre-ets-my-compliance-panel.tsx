...
export function PreEtsMyCompliancePanel() {
  ...
  const load = useCallback(async () => {
    ...
    if (!onlyLate) params.set("onlyLate", "0");
    const qs = params.toString() ? `?${params.toString()}` : "";
    const res = await fetch(`/api/pre-ets/my-compliance${qs}`);
            ...
            {sessions.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-3 py-6 text-center text-brand-black/55">
                  {onlyLate ? "No overdue documentation at your schools." : "No sessions with dates on file."}
                </td>
              </tr>
            ) : (
              sessions.map((s) => (
