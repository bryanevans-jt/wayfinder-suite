...
type PageProps = {
  params: Promise<{ counselorId: string; id: string }>;
};

export default async function GvraSupervisorClientActivityPage({ params }: PageProps) {
  const { counselorId, id: clientId } = await params;
    ...
    if (clientLoadError) {
      return (
        <main className="px-6 py-10">
          <Link
            href={`/dashboard/gvra-supervisor/counselors/${counselorId}`}
            className="text-sm font-medium text-brand-green hover:underline"
          >
            ← Back to {counselorRow.full_name}&apos;s clients
          </Link>
  ...
  return (
    <main className="px-6 py-10">
      ...
      <section className="mx-auto max-w-3xl py-10" aria-labelledby="gvra-client-activity-heading">
        <h2 id="gvra-client-activity-heading" className="text-lg font-semibold text-brand-green">
