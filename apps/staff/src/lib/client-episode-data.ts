...
const EMPTY_PRESENT_STATS = { ratio: 1, totalMinutes: 0, presentMinutes: 0 };
...
export async function loadClientEpisodeContext(clientId: string): Promise<{
  ...
  if (!episodesAvailable) {
    let clientPresentStats = EMPTY_PRESENT_STATS;
    try {
      clientPresentStats = await clientPresentRatioLast30Days(admin, clientId);
    } catch {
      clientPresentStats = EMPTY_PRESENT_STATS;
    }
    return {
      activeEpisodes: [],
      allEpisodes: [],
