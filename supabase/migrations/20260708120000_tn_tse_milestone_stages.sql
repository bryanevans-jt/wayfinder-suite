-- Traditional Supported Employment (TN) uses the same milestone path as TSE (GA).
...
begin
  ...
  for v_rec in
    ...
    where name ilike 'Traditional Supported Employment%'
  loop
    v_is_tse := false;
    v_state := null;
    if v_has_state then
    ...
    v_is_tse :=
      v_rec.name ~ '\(GA\)' or v_rec.name ~ '\(TN\)'
      or upper(coalesce(v_state, '')) in ('GA', 'TN');
